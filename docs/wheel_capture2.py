...
while time.time()<end:
    ...
    while b'\n' in buf:
        ...
        if decode(raw)==5:
            # descrambled address (capture order) + payload
            addr=bytes(raw[i]^scramble[i] for i in range(4)).hex()
            rows.append((ch,addr,brev(raw[4]^scramble[4])))
os.close(fd)
from collections import Counter
print(f"DONE valid packets: {len(rows)}")
print("addresses seen:", dict(Counter(r[1] for r in rows)))
print("payload histogram:", {f'0x{k:02X}':v for k,v in sorted(Counter(r[2] for r in rows).items())})
print("(addr,payload) combos:")
for k,v in sorted(Counter((r[1],r[2]) for r in rows).items(), key=lambda x:-x[1])[:20]:
    print(f"  addr={k[0]} payload=0x{k[1]:02X} x{v}")
